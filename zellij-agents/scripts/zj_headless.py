...
it without putting anything on the user's screen.
    ...
    zj_headless.py agents &

Size defaults deliberately larger than any real terminal. Zellij sizes a session
to its *smallest* attached client, so a big headless client never constrains:
attach with a normal terminal and the session resizes to fit it, and when that
client detaches the session returns to full size. A small headless client would
instead act as a permanent ceiling on what a human sees.
...
def main():
    ...
    session = sys.argv[1]
    cols = int(sys.argv[2]) if len(sys.argv) > 2 else 500
